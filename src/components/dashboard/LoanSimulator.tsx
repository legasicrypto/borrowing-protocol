...
import { calculateMaxBorrowable, calculateHealthFactor, calculateLiquidationPrice } from "@/lib/calculations"
...
export function LoanSimulator() {
  ...
  const { liquidationThreshold, maxLtvBtc, maxLtvUsdc, interestRate, loading: configLoading } = useSystemConfig()
  ...
  const ltvRatio = selectedCrypto === "BTC" ? maxLtvBtc : maxLtvUsdc

  const cryptoPrice = prices[selectedCrypto]?.price_usd || 0
  const priceChange = prices[selectedCrypto]?.change_24h || 0
  const collateralNum = Number.parseFloat(collateralAmount || "0")
  ...
  const liquidationPrice = calculateLiquidationPrice(actualBorrowAmount, collateralNum, liquidationThreshold)
        ...
        <Card className="bg-legasi-card border-2 border-legasi-orange/30">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3">
              <label className="text-sm font-medium text-muted-foreground">Select Collateral</label>
                ...
                {CRYPTOS.map((crypto) => (
                  ...
                  >
                    <div className="flex items-center justify-center gap-2">
                      <img src={crypto.logo || "/placeholder.svg"} alt={crypto.symbol} className="w-6 h-6" />
              ...
              {!loading && prices[selectedCrypto] && (
                <div className="flex items-center gap-2 text-sm p-3 bg-background rounded-lg">
                  ...
                  <span className={`flex items-center gap-1 ${priceChange > 0 ? "text-legasi-green" : "text-red-500"}`}>
            ...
            <div className="space-y-3">
              <label className="text-sm font-medium text-muted-foreground">Collateral Amount</label>
            ...
            {collateralValue > 0 && (
              <div className="space-y-3">
                <label className="text-sm font-medium text-muted-foreground">Amount to Borrow</label>
                <div className="relative">
                  <Input
                    type="number"
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    >
                      {borrowPercentage <= 50 ? "Low Risk" : borrowPercentage <= 75 ? "Medium Risk" : "High Risk"}
                    </span>
                  </div>
                  <div className="h-3 bg-muted rounded-full overflow-hidden">
                    <div
                      className={`h-full transition-all duration-500 ${
                        borrowPercentage <= 50
                          ? "bg-gradient-to-r from-legasi-green to-green-400"
                          : borrowPercentage <= 75
                  ...
                  <div className="flex justify-between items-center text-xs text-muted-foreground">
                    ...
                    <span>
                      {currencySymbol}
                      {actualBorrowAmount.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                    </span>
                    <span>
                      {currencySymbol}
                      {maxBorrowable.toLocaleString(undefined, { maximumFractionDigits: 0 })} max
                    </span>
                  </div>
                </div>
            ...
            {collateralValue > 0 && (
              <div className="space-y-3">
                <label className="text-sm font-medium text-muted-foreground">Choose payout currency</label>
                <div className="grid grid-cols-2 gap-3">
                  <button
                  ...
                  >
                    ...
                    <div className="text-xs text-muted-foreground text-center mt-1">Payout in USD</div>
                  </button>
                  ...
                  >
                    ...
                    <div className="text-xs text-muted-foreground text-center mt-1">Payout in EUR</div>
                  </button>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-4 border-t border-border">
              <div className="p-3 bg-background rounded-lg">
                ...
                <div className="text-2xl font-bold text-legasi-orange">
                  {currencySymbol}
                  {actualBorrowAmount.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                </div>
                <div className="text-xs text-muted-foreground mt-1">
                  of {currencySymbol}
                  {maxBorrowable.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })} max
                </div>
              </div>
              ...
              <div className="p-3 bg-background rounded-lg">
                ...
                <div className={`text-2xl font-bold ${getHealthColor()}`}>{healthFactor.toFixed(2)}</div>
                <div className="text-xs text-muted-foreground mt-1">{getHealthStatus()}</div>
              </div>
            ...
            {/* CTA Button */}
            <GlowButton className="w-full h-12 text-base" onClick={() => navigate("/auth")}>
              Borrow Now
            </GlowButton>
          </CardContent>
        </Card>
