...
interface SwapUsdcToEurModalProps {
  open: boolean;
  onClose: () => void;
  userId: string;
  usdcBalance: number;
...
export function SwapUsdcToEurModal({
  open,
  onClose,
  userId,
  usdcBalance,
  eurBalance,
  onSwapComplete,
}: SwapUsdcToEurModalProps) {
  const [usdcAmount, setUsdcAmount] = useState("");
  const [isProcessing, setIsProcessing] = useState(false);

  const EXCHANGE_RATE = 0.92; // 1 USDC = 0.92 EUR (taux fixe pour simulation)
  const SWAP_FEE_PERCENT = 0.5; // 0.5% de frais

  const calculateEurAmount = (usdc: number) => {
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          usd_balance: newUsdcBalance, // On utilise usd_balance pour stocker USDC
      ...
      toast.success(`${swapAmount} USDC swapped to €${eurAmount.toFixed(2)}!`, {
  ...
  const eurAmount = usdcAmount ? calculateEurAmount(parseFloat(usdcAmount)) : 0;
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="bg-legasi-card border-border">
        <DialogHeader>
          ...
          <DialogDescription>
            Convert your borrowed USDC to EUR before making a bank transfer
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Maximum: {usdcBalance.toFixed(2)} USDC
            </p>
          </div>
          ...
          {usdcBalance <= 0 && (
            <div className="flex items-start gap-2 text-orange-500 text-sm p-3 bg-orange-500/10 border border-orange-500/20 rounded-lg">
              ...
              <span>No USDC available to swap. Create a loan first to receive USDC.</span>
            </div>
          )}
        </div>
