...
interface SwapEurToUsdcModalProps {
  open: boolean;
  onClose: () => void;
  userId: string;
  eurBalance: number;
...
export function SwapEurToUsdcModal({
  open,
  onClose,
  userId,
  eurBalance,
  usdcBalance,
  onSwapComplete,
}: SwapEurToUsdcModalProps) {
  const [eurAmount, setEurAmount] = useState("");
  const [isProcessing, setIsProcessing] = useState(false);

  const EXCHANGE_RATE = 1.087; // 1 EUR = 1.087 USDC (inverse de 0.92)
  const SWAP_FEE_PERCENT = 0.5; // 0.5% de frais

  const calculateUsdcAmount = (eur: number) => {
  ...
  const handleSwap = async () => {
    ...
    try {
      const usdcAmount = calculateUsdcAmount(swapAmount);
      const newEurBalance = eurBalance - swapAmount;
      const newUsdcBalance = usdcBalance + usdcAmount;
      ...
      console.log("Swap transaction inserted:", txData, "Error:", txError);
      ...
      toast.success(`€${swapAmount} swapped to ${usdcAmount.toFixed(2)} USDC!`, {
  ...
  const usdcAmount = eurAmount ? calculateUsdcAmount(parseFloat(eurAmount)) : 0;
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="bg-legasi-card border-border">
        <DialogHeader>
          ...
          <DialogDescription>
            Convert your EUR to USDC to repay your loan or use it elsewhere
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="p-4 bg-background rounded-lg space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold">1 EUR = {EXCHANGE_RATE} USDC</span>
            </div>
            <div className="flex justify-between text-sm">
          ...
          {eurAmount && parseFloat(eurAmount) > 0 && (
            <div className="p-4 bg-legasi-orange/10 border border-legasi-orange/20 rounded-lg space-y-2">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">New USDC balance:</span>
                <span className="font-semibold text-legasi-green">
                  {(usdcBalance + usdcAmount).toFixed(2)} USDC
                </span>
              </div>
            </div>
          )}
          ...
          {eurBalance <= 0 && (
            <div className="flex items-start gap-2 text-orange-500 text-sm p-3 bg-orange-500/10 border border-orange-500/20 rounded-lg">
              ...
              <span>No EUR available to swap. Make a wire transfer or swap USDC to EUR first.</span>
            </div>
          )}
        </div>
