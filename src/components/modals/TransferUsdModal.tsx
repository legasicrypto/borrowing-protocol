...
interface TransferUsdModalProps {
  open: boolean;
  onClose: () => void;
  userId: string;
  ibanPersonal: string;
...
export function TransferUsdModal({
  open,
  onClose,
  userId,
  ibanPersonal,
  usdBalance,
  onTransferComplete,
}: TransferUsdModalProps) {
  const [amount, setAmount] = useState("");
  const [isProcessing, setIsProcessing] = useState(false);

  const FEES = 2; // Fixed fee of $2
  const maxTransferable = Math.max(0, usdBalance - FEES);
    ...
    try {
      ...
      const { data: transfer, error: transferError } = await supabase
        ...
        .insert({
          ...
          amount_eur: transferAmount, // Store as EUR equivalent for now
          fees_eur: FEES,
          status: "pending",
        })
        .select()
      ...
      const { error: updateError } = await supabase
        .from("user_bank_accounts")
        .update({
          usd_fiat_balance: usdBalance - totalCost,
        })
        .eq("user_id", userId);
  ...
  const remainingBalance = usdBalance - totalCost;
      ...
      <DialogContent className="bg-legasi-card border-border">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            USD Transfer to your IBAN
          </DialogTitle>
          <DialogDescription>
            Transfer USD from your Legasi account to your personal IBAN
          </DialogDescription>
        </DialogHeader>
