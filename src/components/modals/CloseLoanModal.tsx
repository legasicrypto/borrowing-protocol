...
export function CloseLoanModal({ open, onClose, onConfirm, loan, currentBalance }: CloseLoanModalProps) {
  if (!loan) return null
  ...
  const getCollateralLogo = () => {
    return loan.collateralType === "BTC" ? btcLogo : usdcLogo
  }
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="bg-legasi-card border-border">
        <DialogHeader>
          ...
          <DialogDescription>This action will fully repay the loan and return your collateral</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="p-4 bg-background rounded-lg space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold">
                {currencySymbol}
                {loan.borrowedEur.toFixed(2)} {currencyLabel}
              </span>
            </div>
            <div className="flex justify-between text-sm">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold">
                {currencySymbol}
                {interestAccrued.toFixed(2)} {currencyLabel}
              </span>
            </div>
            <div className="flex justify-between font-bold text-base pt-2 border-t border-border">
              <span>Total to repay:</span>
              <span className="text-legasi-orange">
                {currencySymbol}
                {totalToRepay.toFixed(2)} {currencyLabel}
              </span>
            </div>
          </div>
          ...
          <div className="p-4 bg-background rounded-lg space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className={`font-semibold ${hasEnoughBalance ? "text-legasi-green" : "text-red-500"}`}>
                {currencySymbol}
                {currentBalance.toFixed(2)} {currencyLabel}
              </span>
            </div>
            {!hasEnoughBalance && (
              <div className="flex justify-between text-sm text-red-500">
                ...
                <span className="font-bold">
                  {currencySymbol}
                  {missingAmount.toFixed(2)} {currencyLabel}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="p-4 bg-legasi-green/10 border border-legasi-green/20 rounded-lg">
            <p className="text-sm text-muted-foreground mb-1">Collateral returned:</p>
            <p className="text-lg font-bold text-legasi-green flex items-center gap-2">
              <img src={getCollateralLogo() || "/placeholder.svg"} alt={loan.collateralType} className="w-5 h-5" />
        ...
        <DialogFooter>
          ...
          <Button variant="destructive" onClick={onConfirm} disabled={!hasEnoughBalance}>
            Confirm Closure
          </Button>
        </DialogFooter>
      </DialogContent>
