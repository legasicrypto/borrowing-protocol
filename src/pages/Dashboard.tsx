...
export default function Dashboard() {
  ...
  const cryptoPricesMap: Record<string, { price_usd: number; price_eur: number }> = {
    SOL: {
      ...
      price_eur: realtimePrices["SOL"]?.price_eur || getPrice("SOL") / 1.1,
    },
    BTC: {
      price_usd: getPrice("BTC"),
      price_eur: realtimePrices["BTC"]?.price_eur || getPrice("BTC") / 1.1,
    },
    ETH: {
      price_usd: getPrice("ETH"),
      price_eur: realtimePrices["ETH"]?.price_eur || getPrice("ETH") / 1.1,
    },
    USDC: {
      price_usd: getPrice("USDC"),
      price_eur: realtimePrices["USDC"]?.price_eur || getPrice("USDC") / 1.1,
    },
  }
  ...
  const handleRepay = async (loanId: string, repayAmount: number) => {
    ...
    const totalInterestAccrued = calculateAccruedInterest(loan.borrowedEur, loan.interestRate, loan.createdAt)
    ...
    toast.success(`Repaid ${repayAmount.toFixed(2)} ${borrowCurrency} + ${proportionalInterest.toFixed(2)} interest`)
  }
  ...
  const handleConfirmCloseLoan = async () => {
    ...
    const { error } = await supabase.from("loan_positions").update({ status: "closed" }).eq("id", selectedLoanId)
  ...
  const generateHistoricalData = () => {
    ...
    const firstLoanDate = new Date(Math.min(...loans.map((loan) => new Date(loan.createdAt).getTime())))

    const today = new Date()
    const monthsDiff = Math.max(
      1,
      (today.getFullYear() - firstLoanDate.getFullYear()) * 12 + (today.getMonth() - firstLoanDate.getMonth()),
    )
  ...
  const collateralDistribution = loans.reduce(
    (acc, loan) => {
  ...
  return (
    <div className="min-h-screen bg-legasi-dark">
      ...
      <div className="max-w-7xl mx-auto p-8">
        ...
        {session?.user?.id && (
          <div className="mb-8">
            <FiatManagement key={fiatKey} userId={session.user.id} onTransactionsUpdate={refreshTransactions} />
          </div>
        )}
                  ...
                  <div className="flex gap-2">
                    {avgHealthFactor < 1.5 && (
                      <Button
                        variant="outline"
                        size="sm"
                        className="flex-1 border-orange-500 text-orange-500 hover:bg-orange-500/10 bg-transparent"
                        onClick={() => {
                    ...
                    {avgLtv > 30 && (
                      <Button
                        variant="outline"
                        size="sm"
                        className="flex-1 border-blue-500 text-blue-500 hover:bg-blue-500/10 bg-transparent"
                        onClick={() => {
      ...
      <CloseLoanModal
        ...
        currentBalance={
          selectedLoanId ? getCurrentBalance(loans.find((l) => l.id === selectedLoanId)?.borrowCurrency || "USDC") : 0
        }
      />
      ...
      <RepayLoanModal
        ...
        currentBalance={
          repayLoanId ? getCurrentBalance(loans.find((l) => l.id === repayLoanId)?.borrowCurrency || "USDC") : 0
        }
      ...
      <AddCollateralModal
        ...
        collateralType={
          addCollateralLoanId ? loans.find((l) => l.id === addCollateralLoanId)?.collateralType || "" : ""
        }
        currentCollateral={
          addCollateralLoanId ? loans.find((l) => l.id === addCollateralLoanId)?.collateralAmount || 0 : 0
        }
        ...
        cryptoPrice={
          addCollateralLoanId ? getPrice(loans.find((l) => l.id === addCollateralLoanId)?.collateralType || "") : 0
        }
