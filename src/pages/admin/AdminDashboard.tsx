...
export default function AdminDashboard() {
  ...
  const loadAdminData = async () => {
    ...
    // Load system config (max LTV for both BTC and USDC, and interest rate)
    const { data: configData, error: configError } = await supabase
      .from("system_config")
      .select("*")
      .in("config_key", ["max_ltv_sol", "max_ltv_btc", "max_ltv_usdc", "interest_rate"])

    if (!configError && configData) {
      configData.forEach((config: any) => {
        // Support both old "max_ltv_sol" and new "max_ltv_btc" keys
        if (config.config_key === "max_ltv_sol" || config.config_key === "max_ltv_btc") {
  ...
  const handleUpdateLtvConfig = async (asset: "BTC" | "USDC") => {
    const configKey = asset === "BTC" ? "max_ltv_btc" : "max_ltv_usdc"
    const newValue = asset === "BTC" ? maxLtvBtc : maxLtvUsdc
  ...
  return (
    <div className="min-h-screen bg-legasi-dark">
      <DashboardHeader title="Admin" showHomeButton={true} showDashboardButton={true} onLogout={handleLogout} />

      <div className="max-w-7xl mx-auto p-8">
        <div className="mb-8">
          <h2 className="text-3xl font-bold mb-2">Admin Dashboard</h2>
          <p className="text-muted-foreground">User, loan, and KYC verification management</p>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <StatCard title="Active Loans" value={stats.activeLoans} icon={DollarSign} />
            <StatCard title="Pending KYC" value={stats.pendingKYC} icon={FileText} />
            <StatCard title="Total Volume" value={stats.totalVolume} prefix="$" decimals={2} icon={Shield} />
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <StatCard
              title="Total BTC Collateral"
              value={stats.totalBtcCollateral}
              suffix=" BTC"
              decimals={2}
              icon={Coins}
            />
            ...
            <StatCard
              ...
              icon={TrendingUp}
            />
            <StatCard title="Average LTV" value={stats.averageLtv} suffix="%" decimals={1} icon={Activity} />
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <StatCard
              ...
              icon={Shield}
            />
            <StatCard title="Active Positions" value={stats.activeLoans} icon={Activity} />
          </div>
        </div>
        ...
        <Tabs defaultValue="users" className="space-y-4">
                  ...
                  <TableBody>
                    {users.map((user) => (
                      <TableRow key={user.id}>
                        ...
                        <TableCell>
                          <RiskBadge status={user.role === "admin" ? "verified" : "active"} />
                        </TableCell>
          ...
          <TabsContent value="loans">
            <Card className="bg-legasi-card border-border">
              <CardHeader>
                ...
                <CardDescription>All active loans on the platform</CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {kycSubmissions.map((kyc) => (
                      <TableRow key={kyc.id}>
                        ...
                        <TableCell>{new Date(kyc.submitted_at).toLocaleDateString("fr-FR")}</TableCell>
                        <TableCell>
                          <div className="flex gap-2">
                            ...
                            {kyc.status === "pending" && (
                              <>
                                ...
                                <Button size="sm" variant="destructive" onClick={() => handleRejectKYC(kyc.id)}>
                                  Reject
                                </Button>
                              </>
                            )}
          ...
          <TabsContent value="config" className="space-y-6">
            <Card className="bg-legasi-card border-border">
              <CardHeader>
                <CardTitle>Bitcoin (BTC) - Maximum LTV</CardTitle>
                <CardDescription>LTV configuration for loans collateralized with BTC (volatile asset)</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="ltv-btc-slider" className="text-base font-semibold">
                      Maximum LTV: {maxLtvBtc}%
                    </Label>
                    <span className="text-2xl font-bold text-legasi-orange">{maxLtvBtc}%</span>
                  </div>
                  ...
                  <div className="space-y-2">
                    <input
                      ...
                      step="5"
                      value={maxLtvBtc}
                      onChange={(e) => setMaxLtvBtc(Number.parseInt(e.target.value))}
                  ...
                  <div className="flex items-start gap-2 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                    ...
                    <div className="space-y-1">
                      <p className="text-sm font-medium text-yellow-500">Volatility Warning</p>
                      <p className="text-xs text-muted-foreground">
                        BTC is volatile. Lower LTV reduces liquidation risk.
                      </p>
                    </div>
                  </div>

                  <Button onClick={() => handleUpdateLtvConfig("BTC")} className="w-full">
                    Update BTC LTV
                  </Button>
                </div>
              </CardContent>
            ...
            <Card className="bg-legasi-card border-border">
              <CardHeader>
                ...
                <CardDescription>LTV configuration for loans collateralized with USDC (stablecoin)</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-2xl font-bold text-legasi-orange">{maxLtvUsdc}%</span>
                  </div>
                  ...
                  <div className="space-y-2">
                    <input
                      ...
                      step="5"
                      value={maxLtvUsdc}
                      onChange={(e) => setMaxLtvUsdc(Number.parseInt(e.target.value))}
                      className="w-full h-2 bg-border rounded-lg appearance-none cursor-pointer accent-legasi-orange"
                    />
                    <div className="flex justify-between text-xs text-muted-foreground">
                      <span>10%</span>
                  ...
                  <div className="flex items-start gap-2 p-4 bg-green-500/10 border border-green-500/20 rounded-lg">
                    ...
                    <div className="space-y-1">
                      <p className="text-sm font-medium text-green-500">Stable Asset</p>
                      <p className="text-xs text-muted-foreground">
                  ...
                  <Button onClick={() => handleUpdateLtvConfig("USDC")} className="w-full">
                    Update USDC LTV
                  </Button>
                </div>
              </CardContent>
            ...
            <Card className="bg-legasi-card border-border">
              <CardHeader>
                ...
                <CardDescription>Interest rate applied to all new loans</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-2xl font-bold text-legasi-orange">{interestRate}%</span>
                  </div>
                  ...
                  <div className="flex items-start gap-2 p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                    ...
                    <div className="space-y-1">
                      <p className="text-sm font-medium text-blue-500">Competitive Rate</p>
                      <p className="text-xs text-muted-foreground">
                  ...
                  <Button onClick={handleUpdateInterestRate} className="w-full">
