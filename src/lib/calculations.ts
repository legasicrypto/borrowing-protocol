 ...
 */
export function calculateHealthFactor(collateralValue: number, borrowed: number, liquidationThreshold: number): number {
 ...
 */
export function calculateMaxBorrowable(collateralValue: number, maxLTV = 75): number {
 ...
 */
export function calculateAccruedInterest(borrowedAmount: number, annualRate: number, createdAt: string): number {
  const now = Date.now()
